...
export const getResources = async (req: CustomRequest, res: Response):Promise<Response> => {
    try {
        ...
        if(!req.id) return res.status(400).json({error: "Unauthorised access to resources"});
...
export const accessResource = async (req: CustomRequest, res: Response):Promise<Response> => {
    try {
        ...
        if(!req.id){
            return res.status(400).json({error: "Unauthorised access to resources"});
        }
        ...
        if(!foundUser) return res.status(500).json({error: "User not found"});

        if(foundUser.leftResources == 0){
            return res.json(400).json({error: "Can't access anymore resources"});
        }
